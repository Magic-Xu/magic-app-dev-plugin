...
Continue independent work. Skill routing does not authorize adjacent work: a review-only audit or analysis
remains read-only; review-and-repair requests include scoped fixes. Implementation does not itself authorize
merge, store upload, or publication. Reuse existing authorization for the requested operation instead of
asking at every step.
